...
from .input_representation import InputTextObj
import torch
...
def get_all_dist(candidate_embeddings_list, text_obj, dist_list):
    ...
    for i, emb in enumerate(candidate_embeddings_list):
        ...
        phrase_type = text_obj.keyphrase_candidate[i][-1]
        phrase = phrase.lower()
        lemma_phrase = wnl.lemmatize(phrase)
        if lemma_phrase not in lemma_phrase_dict:
            lemma_phrase_dict[lemma_phrase] = phrase
        if(lemma_phrase in dist_all):
            #store the No. and distance
            dist_all[lemma_phrase][0].append(dist_list[i])
        else:
            dist_all[lemma_phrase] = ([], phrase_type)
            dist_all[lemma_phrase][0].append(dist_list[i])
    for k, v in lemma_phrase_dict.items():
        if k != v:
            dist_all[v] = dist_all[k]
            del dist_all[k]
    return dist_all
    ...
    if(method=="average"):
        ...
        for phrase, dist_list in dist_all.items():
            ...
            if (phrase in stop_words):
                sum_dist = 0.0
            final_dist[phrase] = []
            final_dist[phrase].append(sum_dist/float(len(dist_list[0])))
            final_dist[phrase].append(dist_list[1])
            final_dist[phrase] = final_dist[phrase]
        return final_dist
...
def SIFRank(text, SIF, en_model, method="average", N=15,
    ...
    dist_sorted = sorted(dist_final.items(), key=lambda x: x[1][0], reverse=True)
...
def SIFRank_plus(text, SIF, en_model, method="average", N=15,
    ...
    for np, dist in dist_final.items():
        if np in position_score:
            dist_final[np][0] = dist[0]*position_score[np]/average_score  # Little change here
    dist_sorted = sorted(dist_final.items(), key=lambda x: x[1][0], reverse=True)
    return dist_sorted[0:N]
