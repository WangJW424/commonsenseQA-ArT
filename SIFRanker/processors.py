...
def get_instance(prompt_temp, gold_ques, choices, gold_ans=None, gen_ques=None, dis_ans=None):
    return {
        ...
        'gold_ans': gold_ans,
        'dis_ans': dis_ans,
        'gen_ques': gen_ques if gen_ques is not None else []
    }


def convert_examples_to_instances(examples, is_training=False):
    all_instances = []
    for example in examples:
        prompt_temp = example.context + ' \"[CHOICE]\" is the answer to the question:'
        gold_ans = None
        dis_ans = None
        if is_training and example.label is not None:
            gold_ans = example.choices[example.label]
            dis_labels = list(range(len(example.choices)))
            dis_labels.remove(example.label)
            dis_ans = example.choices[sample(dis_labels, 1)[0]]
        gold_ques = example.question
        instance = get_instance(prompt_temp, gold_ques, example.choices, gold_ans, example.key_phrases, dis_ans)
...
class InputFeature(object):
    def __init__(self, prompts_for_choices, prompts_for_question=None, prompts_for_context=None,
                 context=None, question=None, choices=None):
        self.prompts_for_choices = prompts_for_choices
        self.prompts_for_question = prompts_for_question
